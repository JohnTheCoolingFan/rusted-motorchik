...
async fn enable_command(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    guild_config
        .edit_command_filter(&command_name, |e| e.enable())
        .await?;
    msg.channel_id
        .say(&ctx.http, format!("Enabled command `{command_name}`"))
        .await?;
    Ok(())
}
...
async fn disable_command(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let command_name = args.quoted().trimmed().single::<String>()?;
    guild_config
        .edit_command_filter(&command_name, |e| e.disable())
        .await?;
    msg.channel_id
        .say(&ctx.http, format!("Disabled command `{command_name}`"))
        .await?;
    Ok(())
}
...
async fn whitelist_command(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let channels: Vec<ChannelId> = args
        .iter::<ChannelId>()
        .quoted()
        .trimmed()
        .collect::<Result<Vec<ChannelId>, ArgError<<ChannelId as FromStr>::Err>>>()?;
    ...
    let mut guild_config = guild_config_arc.write().await;
    guild_config
        .edit_command_filter(&command_name, |e| {
            e.filter_type(CommandDisability::Whitelisted)
                .channels(channels)
        })
        .await?;
    msg.channel_id
        .say(
            &ctx.http,
            format!("Set filtering for command `{command_name}` to whitelist:\n{mentions}"),
        )
        .await?;
    Ok(())
}
...
async fn blacklist_command(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    guild_config
        .edit_command_filter(&command_name, |e| {
            e.filter_type(CommandDisability::Blacklisted)
                .channels(channels)
        })
        .await?;
    msg.channel_id
        .say(
            &ctx.http,
            format!("Set filtering for command `{command_name}` to blacklist:\n{mentions}"),
        )
        .await?;
    Ok(())
}
...
async fn enable_ic(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let mut guild_config = guild_config_arc.write().await;
    guild_config
        .edit(|e| e.info_channel(ic_type, |i| i.state(true)))
        .await?;
    msg.channel_id
        .say(
            &ctx.http,
            format!("Successfully enabled `{}` info channel.", ic_type.as_ref()),
        )
        .await?;
    Ok(())
}
...
async fn disable_ic(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let mut guild_config = guild_config_arc.write().await;
    guild_config
        .edit(|e| e.info_channel(ic_type, |i| i.state(false)))
        .await?;
    msg.channel_id
        .say(
            &ctx.http,
            format!("Successfully disabled `{}` info channel.", ic_type.as_ref()),
        )
        .await?;
    Ok(())
}
...
async fn set_ic(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let mut guild_config = guild_config_arc.write().await;
    guild_config
        .edit(|e| e.info_channel(ic_type, |i| i.channel(channel)))
        .await?;
    msg.channel_id
        .say(
            &ctx.http,
            format!(
                "Channel for `{}` is set to {}",
                ic_type.as_ref(),
                channel.mention()
            ),
        )
...
async fn default_roles(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let roles = args
        ...
        .quoted()
        .trimmed()
        .collect::<Result<Vec<RoleId>, ArgError<<RoleId as FromStr>::Err>>>()?;
    let roles_cnt = roles.len();
    let guild_config_arc =
        GuildConfigManager::get_guild_config_from_ctx(ctx, msg.guild_id.unwrap()).await?;
    let mut guild_config = guild_config_arc.write().await;
    guild_config.edit(|e| e.default_roles(roles)).await?;
    msg.channel_id
        .say(&ctx.http, format!("Default roles ({roles_cnt}) set"))
        .await?;
    Ok(())
}
